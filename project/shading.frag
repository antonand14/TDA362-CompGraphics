...
vec3 calculateDirectIllumiunation(vec3 wo, vec3 n, vec3 base_color)
{
	...
	float brdfdenom = 4.0 * clamp(ndotwo * ndotwi, 0.0001, 1.0);
	float brdf = D * F * G / brdfdenom;
...
vec3 calculateIndirectIllumination(vec3 wo, vec3 n, vec3 base_color)
{
	...
	irradiance = environment_multiplier * textureLod(reflectionMap, lookup, roughness * 7.0).rgb;
	vec3 wh = normalize(wi+wo);
	float wodotwh = max(0.0, dot(wo, wh));
	float F = material_fresnel + (1.0 - material_fresnel) * pow(1.0 - wodotwh, 5.0);
	vec3 dielectric_term = F * irradiance + (1.0 - F) * diffuse_term;
	vec3 metal_term = F * base_color * irradiance;
